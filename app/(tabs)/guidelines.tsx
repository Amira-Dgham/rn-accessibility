...
import { ESSENTIALS_QUESTIONS, PRINCIPLES, RESOURCES, SUCCESS_CRITERIA_LEVELS } from '@/constants/guidelines';
        ...
        <ThemedView preset="scroll" safeAreaEdges={['top']}>
            ...
            <View style={styles.section}>
                <Accordion title={t('screens.guidelines.sections.successCriteria')}>
                    <ThemedText variant="body" style={styles.item}>{t('screens.guidelines.successCriteria.description')}</ThemedText>
                    {SUCCESS_CRITERIA_LEVELS.map((item, idx) => (
                        <ThemedText key={idx} variant="body" style={styles.item}>- {t(item)}</ThemedText>
                    ))}
                </Accordion>
                ...
                <Accordion title={t('screens.guidelines.essentials.title')}>
                    {ESSENTIALS_QUESTIONS.map((q, idx) => (
                        <ThemedText key={idx} variant="body" style={styles.item}>{idx + 1}. {t(q)}</ThemedText>
                    ))}
                </Accordion>
