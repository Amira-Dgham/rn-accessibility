...
interface Props {
  ...
  levels: { key: string; name: string; description: string }[];
  t: (key: string) => string;
  examplesWithComponents: React.ReactNode;
...
const AccessibilityComponentScreen: React.FC<Props> = ({
  ...
  console.log('hell', examplesWithComponents);
  const [internalSelectedLevel, setInternalSelectedLevel] = useState<string>('A');

  // Use external state if provided, otherwise use internal state
  const selectedLevel =
    externalSelectedLevel !== undefined ? externalSelectedLevel : internalSelectedLevel;
